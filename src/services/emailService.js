...
const EmailService = {
  ...
  enviarConfirmacion: async ({ nombre, email, boletos, pins, sorteo_id, monto }) => {
    const boletosHtml = boletos.map((b, i) => `
      <tr>
        <td style="padding:8px 12px; border-bottom:1px solid #eee;">
          <strong>${b}</strong>
        </td>
        <td style="padding:8px 12px; border-bottom:1px solid #eee; color:#C9A84C;">
          <strong>${pins[i]}</strong>
        </td>
      </tr>
    `).join('');
    ...
    <body style="font-family:Arial,sans-serif; background:#f5f5f5; padding:20px;">
      <div style="max-width:600px; margin:0 auto; background:#fff; border-radius:8px; overflow:hidden;">
        ...
        <div style="background:#0D1B3E; padding:30px; text-align:center;">
          ...
          <p style="color:#fff; margin:8px 0 0;">Tus boletos están reservados</p>
        </div>

        <div style="padding:30px;">
          <p>Hola <strong>${nombre}</strong>,</p>
          <p>Tu compra ha sido procesada exitosamente. Guarda bien tu(s) PIN(s) — los necesitarás para reclamar tu premio si resultas ganador.</p>

          <table style="width:100%; border-collapse:collapse; margin:20px 0;">
            <thead>
              <tr style="background:#0D1B3E;">
                <th style="padding:10px 12px; color:#fff; text-align:left;">Número de Boleto</th>
                <th style="padding:10px 12px; color:#fff; text-align:left;">Tu PIN</th>
              </tr>
            </thead>
          ...
          <div style="background:#FFF8E1; border-left:4px solid #C9A84C; padding:15px; margin:20px 0;">
            <strong>⚠️ Importante:</strong> Guarda este correo. Tu PIN es único e irrepetible
            y es el único comprobante válido para reclamar tu premio.
          </div>

          <p><strong>Monto pagado:</strong> $${monto} MXN</p>
          <p><strong>Para reclamar tu premio necesitas:</strong></p>
          <ul>
            <li>Tu PIN de verificación</li>
    ...
    await transporter.sendMail({
      ...
      to:      email,
      subject: `✅ Confirmación de compra — ${boletos.length} boleto(s)`,
      html
    });
  },
